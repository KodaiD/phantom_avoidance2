...
void Result::displayCommitAbortCount(size_t thread_num) {
  std::cout << thread_num << "," << total_commit_counts_ \
                          << "," << total_abort_counts_  \
                          << std::endl;
}

void Result::displayTpsAndAbortRatio(size_t extime, size_t thread_num) {
  ...
  cout << fixed << setprecision(4) << thread_num << "," << result << "," << ave_rate << endl;
}

void Result::displayTpsAndAbortRatioPerOps(size_t extime, size_t thread_num) {
  uint64_t result_read = total_read_commit_counts_ / extime;
  uint64_t result_upsert = total_upsert_commit_counts_ / extime;
  uint64_t result_delete = total_delete_commit_counts_ / extime;
  uint64_t result_range = total_range_commit_counts_ / extime;
  uint64_t result_total = total_commit_counts_ / extime;

  long double ar_read = (double) total_read_abort_counts_ /
                 (double) (total_read_commit_counts_ + total_read_abort_counts_);
  long double ar_upsert = (double) total_upsert_abort_counts_ /
                 (double) (total_upsert_commit_counts_ + total_upsert_abort_counts_);
  long double ar_delete = (double) total_delete_abort_counts_ /
                 (double) (total_delete_commit_counts_ + total_delete_abort_counts_);
  long double ar_range = (double) total_range_abort_counts_ /
                 (double) (total_range_commit_counts_ + total_range_abort_counts_);
  long double ar_total = (double) total_abort_counts_ /
                         (double) (total_commit_counts_ + total_abort_counts_);
  cout << "num_threads,read,upsert,delete,range,total" << endl;
  cout << thread_num << "," << result_read << "," << result_upsert << "," \
       << result_delete << "," << result_range <<  "," << result_total \
       << endl;
  cout << "num_threads,read,upsert,delete,range,total" << endl;
  cout << fixed << setprecision(4) << thread_num << "," \
       << ar_read << "," << ar_upsert << "," \
       << ar_delete << "," << ar_range <<  "," << ar_total \
       << endl;
}

void Result::displayCommitCountPerOps() {
  uint64_t result_read = total_read_commit_counts_;
  uint64_t result_upsert = total_upsert_commit_counts_;
  uint64_t result_delete = total_delete_commit_counts_;
  uint64_t result_range = total_range_commit_counts_;
  cout << "read,upsert,delete,range" << endl;
  cout << result_read << "," << result_upsert << "," \
       << result_delete << "," << result_range << endl;
}

void Result::displayCauseOfAbort() {
  cout << "rv,nv,tuple_die,node_die" << endl;
  cout << total_abort_by_read_validation << "," \
       << total_abort_by_node_validation << "," \
       << total_abort_by_tuple_wait_die << "," \
       << total_abort_by_node_wait_die << endl;
}
...
void Result::addLocalReadCommitCounts(const uint64_t count) {
  total_read_commit_counts_ += count;
}

void Result::addLocalUpsertCommitCounts(const uint64_t count) {
  total_upsert_commit_counts_ += count;
}

void Result::addLocalDeleteCommitCounts(const uint64_t count) {
  total_delete_commit_counts_ += count;
}

void Result::addLocalRangeCommitCounts(const uint64_t count) {
  total_range_commit_counts_ += count;
}

void Result::addLocalReadAbortCounts(const uint64_t count) {
  total_read_abort_counts_ += count;
}

void Result::addLocalUpsertAbortCounts(const uint64_t count) {
  total_upsert_abort_counts_ += count;
}

void Result::addLocalDeleteAbortCounts(const uint64_t count) {
  total_delete_abort_counts_ += count;
}

void Result::addLocalRangeAbortCounts(const uint64_t count) {
  total_range_abort_counts_ += count;
}

void Result::addLocalAbortByReadValidation(const uint64_t count) {
  total_abort_by_read_validation += count;
}

void Result::addLocalAbortByNodeValidation(const uint64_t count) {
  total_abort_by_node_validation += count;
}

void Result::addLocalAbortByTupleWaitDie(const uint64_t count) {
  total_abort_by_tuple_wait_die += count;
}

void Result::addLocalAbortByNodeWaitDie(const uint64_t count) {
  total_abort_by_node_wait_die += count;
}
...
void Result::addLocalAllResult(const Result &other) {
  ...
  addLocalReadCommitCounts(other.local_read_commit_counts_);
  addLocalUpsertCommitCounts(other.local_upsert_commit_counts_);
  addLocalDeleteCommitCounts(other.local_delete_commit_counts_);
  addLocalRangeCommitCounts(other.local_range_commit_counts_);
  addLocalReadAbortCounts(other.local_abort_by_read);
  addLocalUpsertAbortCounts(other.local_abort_by_upsert);
  addLocalDeleteAbortCounts(other.local_abort_by_delete);
  addLocalRangeAbortCounts(other.local_abort_by_range);
  addLocalAbortByReadValidation(other.local_abort_by_read_validation);
  addLocalAbortByNodeValidation(other.local_abort_by_node_validation);
  addLocalAbortByTupleWaitDie(other.local_abort_by_tuple_wait_die);
  addLocalAbortByNodeWaitDie(other.local_abort_by_node_wait_die);
#if ADD_ANALYSIS
