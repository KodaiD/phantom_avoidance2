...
public:
  ...
  ReadElement(uint64_t key, T *rcdptr, char *val, Tidword tidword)
    ...
    if (val) memcpy(this->val_, val, VAL_SIZE);
  }
...
public:
  ...
  WriteElement(uint64_t key, T *rcdptr, std::string_view val, WriteType wtype, bool check)
          : OpElement<T>::OpElement(key, rcdptr), wtype(wtype), check(check) {
