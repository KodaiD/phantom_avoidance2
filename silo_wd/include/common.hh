...
alignas(CACHE_LINE_SIZE) GLOBAL uint64_t_64byte ReclamationEpoch(1);
#if MASSTREE_USE
...
alignas(CACHE_LINE_SIZE) GLOBAL uint64_t_64byte ReclamationEpoch;
#if MASSTREE_USE
alignas(CACHE_LINE_SIZE) GLOBAL MasstreeWrapper<Tuple> MT;
#endif
#endif
...
#ifdef GLOBAL_VALUE_DEFINE
DEFINE_uint64(num_execution, 10, "number of execution.");
...
DEFINE_uint64(range_size, 10000, "number of tuples a range scan operation reads.");
DEFINE_uint64(priority, 50, "priority of range scan. 0 ~ 100");
...
DECLARE_uint64(range_size);
DECLARE_uint64(priority);
...
alignas(CACHE_LINE_SIZE) GLOBAL WaitDieNode *ThreadLocalTupleWaiters;
alignas(CACHE_LINE_SIZE) GLOBAL WaitDieNode *ThreadLocalWaiters;
alignas(CACHE_LINE_SIZE) GLOBAL std::vector<WaitDieNode*> *ThreadLocalOwners;
