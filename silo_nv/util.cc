...
bool chkEpochLoaded() {
  ...
  //     return false;
  // }
  uint64_t min_ew = nowepo;
  uint64_t candidate;
  for (unsigned int i = 1; i < FLAGS_thread_num; ++i) {
    candidate = __atomic_load_n(&(ThLocalEpoch[i].obj_), __ATOMIC_ACQUIRE);
    if (min_ew > candidate) min_ew = candidate;
  }
  if (min_ew != nowepo) return false;
  return true;
}
...
void leaderWork(uint64_t &epoch_timer_start, uint64_t &epoch_timer_stop) {
  ...
  if (chkClkSpan(epoch_timer_start, epoch_timer_stop,
      ...
      chkEpochLoaded()) {
    uint64_t latest = atomicAddGE();
    atomicStoreReclamationEpoch(latest);
