...
void worker(int thid, char& ready, const bool& start, const bool& quit, ArrayIndex& array_index) {
  ...
  while (!loadAcquire(quit)) {
    makeProcedure2(trans.pro_set, rnd, zipf, FLAGS_thread_num, thid, FLAGS_tuple_num, FLAGS_max_ope, FLAGS_rratio, FLAGS_range_size, 0, myres);
#if PROCEDURE_SORT
    ...
    for (auto itr = trans.pro_set.begin(); itr != trans.pro_set.end(); ++itr) {
      switch (itr->ope_) {
        ...
        case Ope::DELETE_IF_EXISTS:
          trans.delete_if_exists(itr->key_);
          break;
        case Ope::RANGE_SCAN:
