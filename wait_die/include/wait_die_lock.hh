...
struct Node {
  using TS = Timestamp;
  Node() {}
  Node(TS ts, Operation op, bool waiting)
...
#define GLOBAL_OWNER
alignas(CACHE_LINE_SIZE) GLOBAL_OWNER std::vector<Node*>** GlobalOwnerPool;
#else
...
struct NodeList {
 ...
 public:
  ...
  void insert(TS ts, Node* new_item) {
    new_item->ts = ts;
    Node* cur;
...
class WaitDieLock {
 private:
   ...
   NodeList waiter_list;
   NodeList owner_list;
 public:
  WaitDieLock() { latch.init(); }

  bool try_lock_shared(Timestamp ts, Node* wn) {
    wn->ts = ts;
    wn->op = SHARED;
    wn->waiting = true;
  ...
  bool try_lock(Timestamp ts, Node* wn) {
    wn->ts = ts;
    wn->op = EXCLUSIVE;
    wn->waiting = true;
    while (!latch.w_trylock()) usleep(1);
  ...
  bool try_lock_upgrade(Timestamp ts, Node* wn) {
    wn->ts = ts;
    wn->op = UPGRADE;
    wn->waiting = true;
    while (!latch.w_trylock()) usleep(1);
  ...
  Node* get_owner(Timestamp ts) {
    ...
    std::runtime_error("ran out of OwnerNode");
    return nullptr;
  }

  void return_owner(TS ts, Node* on) {
    on->next = nullptr;
    on->prev = nullptr;
    auto local_owner_pool = GlobalOwnerPool[ts.thid_];
    for (int i = 0; i < local_owner_pool->size(); i++) {
    ...
    std::runtime_error("full with OwnerNode");
  }
