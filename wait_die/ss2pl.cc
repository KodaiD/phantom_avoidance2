...
int main(int argc, char *argv[]) try {
  ...
  ThreadLocalWaiters = new Node[FLAGS_thread_num];
  ThreadLocalOwners = new std::vector<Node*>[FLAGS_thread_num];
  GlobalOwnerPool = new std::vector<Node*>*[FLAGS_thread_num];

  for (int i = 0; i < FLAGS_thread_num; i++) {
    for (int j = 0; j < FLAGS_max_ope; j++) {
      ThreadLocalOwners[i].emplace_back(new Node());
    }
