...
void TxExecutor::read(uint64_t key) {
  ...
  if (tuple->lock_->try_lock_shared(ts, &ThreadLocalWaiters[thid_])) {
...
void TxExecutor::write(uint64_t key) {
  ...
  for (auto rItr = read_set_.begin(); rItr != read_set_.end(); ++rItr) {
    if ((*rItr).key_ == key) {  // hit
      if (!(*rItr).rcdptr_->lock_->try_lock_upgrade(ts, &ThreadLocalWaiters[thid_])) {
        this->status_ = TransactionStatus::aborted;
        goto FINISH_WRITE;
      }
  ...
  if (!tuple->lock_->try_lock(ts, &ThreadLocalWaiters[thid_])) {
    /**
     * No-wait and abort.
     */
    this->status_ = TransactionStatus::aborted;
