...
inline static void makeProcedure2(std::vector<Procedure> &pro, Xoroshiro128Plus &rnd,
                                 FastZipf &zipf, size_t num_threads, size_t thid,
                                 size_t tuple_num, size_t max_ope, size_t rratio,
                                 uint64_t range, uint64_t priority,
  ...
  //   if (rnd.next() % 100 < rratio) {
  //     uint64_t lkey;
  //     lkey = zipf() % (tuple_num - range);
  //     wonly_flag = false;
  //     pro.emplace_back(Ope::RANGE_SCAN, lkey);
  //   } else {
  //     uint64_t tmpkey;
  //     // decide access destination key.
  //     tmpkey = zipf() % tuple_num;
  //     // decide operation type.
  //     auto rnd_num = rnd.next();
  //     if (rnd_num % 100 < 50) {
  //       ronly_flag = false;
  //       pro.emplace_back(Ope::UPSERT, tmpkey);
  //     } else if (rnd_num % 100 < 100) {
  //       ronly_flag = false;
  //       pro.emplace_back(Ope::DELETE_IF_EXISTS, tmpkey);
  //     } else if (rnd_num % 100 < 100) {
  //       wonly_flag = false;
  //       pro.emplace_back(Ope::READ, tmpkey);
  //     }
  ...
  if (priority == 100) {
    // write 優先
    if (thid < num_threads * (100 - rratio) / 100) {
      for (size_t i = 0; i < max_ope; i++) {
        uint64_t tmpkey;
        // decide access destination key.
        tmpkey = zipf() % tuple_num;
    ...
    // range 優先
    if (thid < num_threads * rratio / 100) {
      uint64_t lkey;
      lkey = zipf() % (tuple_num - range);
      wonly_flag = false;
      pro.emplace_back(Ope::RANGE_SCAN, lkey);
    ...
    std::runtime_error("invalid priority");
  }
